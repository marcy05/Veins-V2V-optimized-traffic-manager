	...
	if (statistics.startTime != simTime()) {
		...
		if (speed != -1) {
			...
			if (last_speed != -1) {
				...
				m_accelVectorFiltering.push_back(acceleration);
				if(m_accelVectorFiltering.size() == 7){
				    double average = 0;
				    for(std::vector<double>::iterator it = m_accelVectorFiltering.begin(); it != m_accelVectorFiltering.end(); ++it){
				        average += *it;
				    }
				    average = average/7;
				    m_acceleration = average;
				    std::vector<double>::iterator it = m_accelVectorFiltering.begin();
				    m_accelVectorFiltering.erase(it);
				}
				//m_acceleration = acceleration; // Extraction of the vehicle acceleration
